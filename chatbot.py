...
def render_header():
    # Use Streamlit's header and subheader for better alignment and responsiveness
    st.markdown("<h2 style='text-align: center; color: #2b6777;'>Confluence Chatbot</h1>", unsafe_allow_html=True)
    st.markdown("<h4 style='text-align: center; color: #4a4a4a;'>Powered by LLM - Your EDP and ML Knowledge Assistant</h3>", unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        **Purpose**: Answer tech-related queries about the EDP department, ML topics, and team FAQs.
        ...
        if st.session_state.messages:
            ...
            file_format = st.selectbox("Select Format", ["Text", "JSON"], key="download_format")
            # Prepare download
            chat_data, file_name, mime_type = prepare_download(st.session_state.messages, file_format.lower())
            # Download button
            st.download_button(
                label=f"Download as {file_format.upper()}",
                data=chat_data,
...
def prepare_download(messages, file_format):
    # Prepare downloadable chat history in the selected format
    if file_format == "json":
        chat_data = json.dumps(messages, indent=4)
        mime_type = "application/json"
        file_name = f"chat_history_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def render_chat():
    ...
    # Accept user input
    if prompt := st.chat_input("Ask your query about EDP or ML:"):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Typing..."):
                time.sleep(1)  # Use time.sleep instead of st.sleep
...
def generate_response(user_query):
    # Simulate an AI assistant response (replace this with LLM integration)
    if "ml" in user_query.lower() or "machine learning" in user_query.lower():
        return "Machine Learning in EDP involves cutting-edge initiatives like reinforcement learning, generative AI, and anomaly detection pipelines."
    elif "edp" in user_query.lower():
        return "The EDP department focuses on delivering scalable machine learning solutions for financial operations."
    else:
